...
Article.prototype.insertRecord = function(callback) {
  $.post('/articles', {author: this.author, authorUrl: this.authorUrl, body: this.body, category: this.category, publishedOn: this.publishedOn, title: this.title})
    .then(data => {
      ...
      // conditional statement, that checks if call back is passed in as a parameter above then the callback will be invoked by this line. If callback is not being passed then the method will not be invoked because it is a conditional and checking if it is truthy.
